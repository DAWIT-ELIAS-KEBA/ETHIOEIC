...
<div class="card custom-card">
    ...
    @if(Auth::check() && Auth::user()->hasPermissionWithName("woreda_registration"))
            <li class="nav-item">
                <a class="nav-link active" data-bs-toggle="tab" href="#registrationTab">Register Woreda</a>
            </li>
    @endif
        ...
        <div class="tab-pane fade show active" id="registrationTab">
            ...
            <div class="row gy-3">
                  ...
                  <div class="input-group mb-3"> 
                    <label class="input-group-text" for="zone_id">Select Zone</label>
...
<script>
    ...
    columns: [
        ...
        { data: 'zone.name', name: 'zone.name' }, // Use dot notation to access the region name
        {
            ...
            render: function (data, type, row) {
                @if (Auth::check() && Auth::user()->hasPermissionWithName("update_woreda"))
                    return '<a href="#" class="btn btn-sm btn-primary edit-woreda" data-id="' + row.id + '">Edit</a>';
                @else
                    return '';
                @endif
            },
        },
        {
            ...
            render: function (data, type, row) {
                @if (Auth::check() && Auth::user()->hasPermissionWithName("delete_woreda"))
                    return '<button class="btn btn-sm btn-danger delete-woreda" data-id="' + row.id + '">Delete</button>';
                @else
                    return '';
                @endif
            },
    ...
    function loadZones() {
        $.ajax({
            type: 'GET',
            url: '/zones/list', // Replace with the correct URL to fetch regions data
            data: {
                _token: '{{ csrf_token() }}',
            },
            success: function (response) {
                var zones = response.data;
                var selectOptions = '<option value="">Select Zones</option>';
                zones.forEach(function (zone) {
                    selectOptions += '<option value="' + zone.id + '">' + zone.name + '</option>';
                });
                $('#zone_id').html(selectOptions);
            },
            error: function (xhr) {
    ...
    $('#woredasTable').on('click', '.delete-woreda', function () {
        ...
        var confirmation = confirm('Are you sure you want to delete this Woreda?');
    ...
    $.ajax({
        ...
        error: function (xhr) {
            if (xhr.status === 422) {
                alert('Error occurred during Woreda registration.');
                var errors = xhr.responseJSON.errors;
                $.each(errors, function (key, value) {
                    alert(value[0]);
                });
