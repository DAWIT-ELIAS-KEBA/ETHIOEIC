...
<div class="card custom-card">
    ...
    @if(Auth::check() && Auth::user()->hasPermissionWithName("region_registration"))
            <li class="nav-item">
                <a class="nav-link active" data-bs-toggle="tab" href="#registrationTab">Register Region</a>
            </li>
    @endif
        ...
        columns: [
            ...
            { data: 'type', name: 'type' },
            {
                ...
                render: function (data, type, row)
                 {
                        @if (Auth::check() && Auth::user()->hasPermissionWithName("update_region"))
                        return '<a href="#" class="btn btn-sm btn-primary edit-region" data-id="' + row.id + '">Edit</a>';
                        @else
                            return ''; 
                        @endif
...

            {
                ...
                render: function (data, type, row) 
                {
                        @if (Auth::check() && Auth::user()->hasPermissionWithName("delete_region"))
                         return '<button class="btn btn-sm btn-danger delete-region" data-id="' + row.id + '">Delete</button>';
                        @else
                            return ''; 
                        @endif
                }
